...
!
      ...
      !  Check if a previous defined bitmap exists
        if (isecnum.eq.6) then
          call g2lib_gbyte(cgrib,ibmprev,iofst,8)
          iofst=iofst+8
...
!
      ...
      call g2lib_sbyte(cgrib,four,iofst,8)     ! Store section number ( 4 )
      iofst=iofst+8
      call g2lib_sbyte(cgrib,numcoord,iofst,16)   ! Store num of coordinate values
      iofst=iofst+16
      call g2lib_sbyte(cgrib,ipdsnum,iofst,16)    ! Store Prod Def Template num.
      iofst=iofst+16
      !
      ...
      do i=1,mappdslen
        ...
        if ( (mappds(i).ge.0).or.(ipdstmpl(i).ge.0) ) then
          call g2lib_sbyte(cgrib,ipdstmpl(i),iofst,nbits)
        else
          call g2lib_sbyte(cgrib,one,iofst,1)
          call g2lib_sbyte(cgrib,iabs(ipdstmpl(i)),iofst+1,nbits-1)
        endif
        iofst=iofst+nbits
      enddo
      !
      ...
      if ( numcoord .ne. 0 ) then
        ...
        call g2lib_sbytes(cgrib,coordieee,iofst,32,0,numcoord)
      ...
      call g2lib_sbyte(cgrib,lensec4,ibeg,32)
!
...
!
      ...
      elseif (idrsnum.eq.50) then      !  Sperical Harmonic Simple Packing
        ...
        !call g2lib_gbyte(re00,idrstmpl(5),0,32)
      ...
      elseif (idrsnum.eq.40 .OR. idrsnum.eq.40000) then     !  JPEG2000 encoding
        ...
        lcpack=nsize
        call jpcpack(pfld,width,height,idrstmpl,cpack,lcpack,ierr)
      else
...
!
      ...
      iofst=ibeg+32         !   leave space for length of section
      call g2lib_sbyte(cgrib,five,iofst,8)     ! Store section number ( 5 )
      iofst=iofst+8
      call g2lib_sbyte(cgrib,ndpts,iofst,32)    ! Store num of actual data points
      iofst=iofst+32
      call g2lib_sbyte(cgrib,idrsnum,iofst,16)    ! Store Data Repr. Template num.
      iofst=iofst+16
      !
      ...
      do i=1,mapdrslen
        ...
        if ( (mapdrs(i).ge.0).or.(idrstmpl(i).ge.0) ) then
          call g2lib_sbyte(cgrib,idrstmpl(i),iofst,nbits)
        else
          call g2lib_sbyte(cgrib,one,iofst,1)
          call g2lib_sbyte(cgrib,iabs(idrstmpl(i)),iofst+1,nbits-1)
        endif
        iofst=iofst+nbits
      enddo
      !
      ...
      call g2lib_sbyte(cgrib,lensec5,ibeg,32)
...
!
      ...
      iofst=ibeg+32         !   leave space for length of section
      call g2lib_sbyte(cgrib,six,iofst,8)     ! Store section number ( 6 )
      iofst=iofst+8
      call g2lib_sbyte(cgrib,ibmap,iofst,8)    ! Store Bit Map indicator
      iofst=iofst+8
      !
      !  Store bitmap, if supplied
      !
      if (ibmap.eq.0) then
        call g2lib_sbytes(cgrib,intbmap,iofst,1,0,ngrdpts)    ! Store BitMap
        iofst=iofst+ngrdpts
      endif
      !
     ...
     &        ' but one does not exist in the current GRIB message.'
      ...
      left=8-mod(iofst,8)
      if (left.ne.8) then
        call g2lib_sbyte(cgrib,zero,iofst,left)     ! Pad with zeros to fill Octet
        iofst=iofst+left
      endif
      lensec6=(iofst-ibeg)/8
      call g2lib_sbyte(cgrib,lensec6,ibeg,32)
...
!
      ...
      iofst=ibeg+32         !   leave space for length of section
      call g2lib_sbyte(cgrib,seven,iofst,8)     ! Store section number ( 7 )
      iofst=iofst+8
      ...
      call g2lib_sbyte(cgrib,lensec7,ibeg,32)
...
!
      ...
      call g2lib_sbyte(cgrib,newlen,96,32)
